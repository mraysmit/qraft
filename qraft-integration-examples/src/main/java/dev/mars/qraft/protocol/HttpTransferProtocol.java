package dev.mars.qraft.protocol;
...
import dev.mars.qraft.core.TransferDirection;
...
import dev.mars.qraft.core.exceptions.QraftErrorCode;
import dev.mars.qraft.core.exceptions.TransferException;
import dev.mars.qraft.storage.ChecksumCalculator;
import dev.mars.qraft.transfer.ProgressTracker;
import dev.mars.qraft.transfer.TransferContext;

import static dev.mars.qraft.core.exceptions.QraftErrorCode.*;
...
public class HttpTransferProtocol implements TransferProtocol {
    ...
    public io.vertx.core.Future<TransferResult> transferReactive(TransferRequest request, TransferContext context) {
        ...
        try {
            ...
            logger.error("[{}] HTTP request validation failed: jobId={}, error={}", QRAFT_1201.code(), context.getJobId(), e.getMessage());
    ...
    private io.vertx.core.Future<TransferResult> performHttpDownload(
        ...
        try {
            ...
            try {
                ...
                logger.error("[{}] HTTP destination directory creation failed: jobId={}, error={}", QRAFT_1202.code(), context.getJobId(), e.getMessage());
            ...
            return webClient.getAbs(url)
                ...
                .compose(response -> {
                    ...
                    if (response.statusCode() != 200) {
                        logger.error("[{}] HTTP download failed: statusCode={}, statusMessage={}", QRAFT_1203.code(),
                                    response.statusCode(), response.statusMessage());
                    ...
                    if (body == null) {
                        logger.error("[{}] HTTP download failed: empty response body", QRAFT_1204.code());
                    ...
                    if (request.getExpectedChecksum() != null && !request.getExpectedChecksum().isEmpty()) {
                        if (!request.getExpectedChecksum().equals(actualChecksum)) {
                            logger.error("[{}] HTTP download checksum mismatch: expected={}, actual={}", QRAFT_1205.code(),
                ...
                .recover(err -> {
                    logger.error("[{}] HTTP download pipeline failed: jobId={}, error={}", QRAFT_1206.code(), context.getJobId(), err.getMessage());
                    logger.debug("HTTP download exception details for job: {}", context.getJobId(), err);
                    return io.vertx.core.Future.failedFuture(err);
                });
        } catch (Exception e) {
            logger.error("[{}] HTTP download setup failed: error={}", QRAFT_1207.code(), e.getMessage());
            return io.vertx.core.Future.failedFuture(e);
        }
    }
    ...
    private io.vertx.core.Future<TransferResult> performHttpUpload(
        ...
        try {
            ...
            if (!Files.exists(sourcePath)) {
                logger.error("[{}] HTTP upload source file not found: {}", QRAFT_1208.code(), sourcePath);
            ...
            return vertx.fileSystem().readFile(sourcePath.toString())
                .compose(fileContent -> {
                    ...
                    if (request.getExpectedChecksum() != null && !request.getExpectedChecksum().isEmpty()) {
                        if (!request.getExpectedChecksum().equals(actualChecksum)) {
                            logger.error("[{}] HTTP upload checksum mismatch: expected={}, actual={}", QRAFT_1209.code(),
                                        request.getExpectedChecksum(), actualChecksum);
                            return io.vertx.core.Future.failedFuture(new TransferException(context.getJobId(),
                                "Checksum mismatch - expected: " + request.getExpectedChecksum() +
                                ", actual: " + actualChecksum));
                    ...
                    return webClient.putAbs(url)
                        ...
                        .map(response -> {
                            ...
                            if (statusCode != 200 && statusCode != 201 && statusCode != 204) {
                                logger.error("[{}] HTTP PUT failed: statusCode={}, statusMessage={}", QRAFT_1210.code(),
                                            statusCode, response.statusMessage());
                ...
                .recover(err -> {
                    logger.error("[{}] HTTP upload pipeline failed: jobId={}, error={}", QRAFT_1211.code(), context.getJobId(), err.getMessage());
            ...
            logger.error("[{}] HTTP upload setup failed: error={}", QRAFT_1212.code(), e.getMessage());
            return io.vertx.core.Future.failedFuture(e);
        }
    }
    ...
    private void validateRequest(TransferRequest request) throws TransferException {
        ...
        if (request.getSourceUri() == null) {
            logger.error("[{}] HTTP validation: source URI is null", QRAFT_1213.code());
        ...
        if (direction == TransferDirection.DOWNLOAD) {
            if (request.getDestinationPath() == null) {
                logger.error("[{}] HTTP validation: destination path is null for download", QRAFT_1214.code());
            ...
            if (request.getDestinationUri() == null) {
                logger.error("[{}] HTTP validation: destination URI is null for upload", QRAFT_1215.code());
                throw new TransferException(request.getRequestId(), "Destination URI cannot be null for upload");
            }
        }
        
        if (!canHandle(request)) {
            logger.error("[{}] HTTP validation: protocol cannot handle this request type", QRAFT_1216.code());
