...
package dev.mars.qraft.tenant.observability;
...
/**
 * OpenTelemetry metrics for Qraft Tenant module.
 * Phase 7 of the OpenTelemetry migration.
 * 
 * Provides 10 tenant-specific metrics:
 * - qraft.tenant.total (gauge) - Total number of tenants
 * - qraft.tenant.active (gauge) - Number of active tenants
 * - qraft.tenant.created (counter) - Total tenants created
 * - qraft.tenant.deleted (counter) - Total tenants deleted
 * - qraft.tenant.quota.violations (counter) - Quota violation attempts
 * - qraft.tenant.resource.reservations (counter) - Resource reservation attempts
 * - qraft.tenant.resource.releases (counter) - Resource releases
 * - qraft.tenant.resource.operations (counter) - Total resource management operations
 * - qraft.tenant.resource.concurrent_transfers (gauge) - Current concurrent transfers per tenant
 * - qraft.tenant.resource.bandwidth (gauge) - Current bandwidth usage per tenant
 *
...
public class TenantMetrics {
    ...
    private static final String METER_NAME = "qraft-tenant";
    ...
    private TenantMetrics() {
        ...
        // Initialize counters
        tenantsCreated = meter.counterBuilder("qraft.tenant.created")
                .setDescription("Total number of tenants created")
                .setUnit("1")
                .build();

        tenantsDeleted = meter.counterBuilder("qraft.tenant.deleted")
                .setDescription("Total number of tenants deleted")
                .setUnit("1")
                .build();

        quotaViolations = meter.counterBuilder("qraft.tenant.quota.violations")
                .setDescription("Number of quota violation attempts")
                .setUnit("1")
                .build();

        resourceReservations = meter.counterBuilder("qraft.tenant.resource.reservations")
                .setDescription("Number of resource reservation attempts")
                .setUnit("1")
                .build();

        resourceReleases = meter.counterBuilder("qraft.tenant.resource.releases")
                .setDescription("Number of resource releases")
                .setUnit("1")
                .build();

        resourceOperations = meter.counterBuilder("qraft.tenant.resource.operations")
                .setDescription("Total resource management operations processed")
                .setUnit("1")
                .build();

        // Initialize gauges
        meter.gaugeBuilder("qraft.tenant.total")
                .setDescription("Total number of tenants in the system")
                .ofLongs()
                .buildWithCallback(measurement -> measurement.record(totalTenants.get()));

        meter.gaugeBuilder("qraft.tenant.active")
        ...
        meter.gaugeBuilder("qraft.tenant.resource.concurrent_transfers")
        ...
        meter.gaugeBuilder("qraft.tenant.resource.bandwidth")
