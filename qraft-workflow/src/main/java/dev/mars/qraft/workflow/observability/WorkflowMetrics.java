...
package dev.mars.qraft.workflow.observability;
...
/**
 * OpenTelemetry metrics for Qraft Workflow Engine.
 * Phase 9 of the OpenTelemetry migration.
 * 
 * Provides 9 workflow-specific metrics:
 * - qraft.workflow.active (gauge) - Currently active workflow executions
 * - qraft.workflow.total (counter) - Total workflows started
 * - qraft.workflow.completed (counter) - Successfully completed workflows
 * - qraft.workflow.failed (counter) - Failed workflows
 * - qraft.workflow.cancelled (counter) - Cancelled workflows
 * - qraft.workflow.steps.total (counter) - Total workflow steps executed
 * - qraft.workflow.steps.failed (counter) - Failed workflow steps
 * - qraft.workflow.duration.seconds (histogram) - Workflow duration distribution
 * - qraft.workflow.transfers.per_workflow (histogram) - Transfers per workflow
 *
...
public class WorkflowMetrics {
    ...
    private static final String METER_NAME = "qraft-workflow";
    ...
    private WorkflowMetrics() {
        ...
        // Initialize counters
        workflowsTotal = meter.counterBuilder("qraft.workflow.total")
                .setDescription("Total number of workflows started")
                .setUnit("1")
                .build();

        workflowsCompleted = meter.counterBuilder("qraft.workflow.completed")
                .setDescription("Number of successfully completed workflows")
                .setUnit("1")
                .build();

        workflowsFailed = meter.counterBuilder("qraft.workflow.failed")
                .setDescription("Number of failed workflows")
                .setUnit("1")
                .build();

        workflowsCancelled = meter.counterBuilder("qraft.workflow.cancelled")
                .setDescription("Number of cancelled workflows")
                .setUnit("1")
                .build();

        stepsTotal = meter.counterBuilder("qraft.workflow.steps.total")
                .setDescription("Total number of workflow steps executed")
                .setUnit("1")
                .build();

        stepsFailed = meter.counterBuilder("qraft.workflow.steps.failed")
                .setDescription("Number of failed workflow steps")
                .setUnit("1")
                .build();

        // Initialize histograms
        workflowDuration = meter.histogramBuilder("qraft.workflow.duration.seconds")
                .setDescription("Workflow duration in seconds")
                .setUnit("s")
                .build();

        transfersPerWorkflow = meter.histogramBuilder("qraft.workflow.transfers.per_workflow")
                .setDescription("Number of transfers per workflow")
                .setUnit("1")
                .build();

        // Initialize gauges
        meter.gaugeBuilder("qraft.workflow.active")
