...
public class FtpTransferProtocol implements TransferProtocol {
    ...
    public TransferResult transfer(TransferRequest request, TransferContext context) throws TransferException {
        ...
        try {
            ...
            logger.error("[{}] FTP transfer failed: jobId={}, error={}", QRAFT_1000.code(), context.getJobId(), e.getMessage());
    ...
    private TransferResult performFtpDownload(TransferRequest request, ProgressTracker progressTracker) 
        ...
        try {
            ...
            logger.error("[{}] FTP download failed: requestId={}, error={}", QRAFT_1001.code(), requestId, e.getMessage());
    ...
    private TransferResult performFtpUpload(TransferRequest request, ProgressTracker progressTracker) 
        ...
        try {
            ...
            if (!Files.exists(sourcePath)) {
                logger.error("[{}] FTP upload source file not found: {}", QRAFT_1003.code(), sourcePath);
            ...
            logger.error("[{}] FTP upload failed: requestId={}, error={}", QRAFT_1002.code(), requestId, e.getMessage());
    ...
    private static class FtpClient {
        ...
        void connect() throws IOException {
            ...
            if (!response.startsWith("220")) {
                logger.error("[{}] FTP connection rejected by server: {}", QRAFT_1004.code(), response);
            ...
            if (!response.startsWith("230")) {
                logger.error("[{}] FTP authentication failed: {}", QRAFT_1005.code(), response);
            ...
            if (!response.startsWith("200")) {
                logger.error("[{}] FTP binary mode setup failed: {}", QRAFT_1006.code(), response);
        ...
        private String readResponse() throws IOException {
            ...
            if (response == null) {
                logger.error("[{}] FTP server closed connection unexpectedly (EOF)", QRAFT_1009.code());
