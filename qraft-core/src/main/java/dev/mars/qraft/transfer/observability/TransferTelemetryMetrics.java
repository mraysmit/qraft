...
package dev.mars.qraft.transfer.observability;
...
/**
 * OpenTelemetry metrics for Qraft Core Transfer Engine.
 ...
 * - qraft.transfer.active (gauge) - Currently active transfers
 * - qraft.transfer.total (counter) - Total transfers initiated
 * - qraft.transfer.completed (counter) - Successfully completed transfers
 * - qraft.transfer.failed (counter) - Failed transfers
 * - qraft.transfer.cancelled (counter) - Cancelled transfers
 * - qraft.transfer.bytes.total (counter) - Total bytes transferred
 * - qraft.transfer.duration.seconds (histogram) - Transfer duration distribution
 * - qraft.transfer.throughput.bytes_per_second (histogram) - Throughput distribution
 * - qraft.transfer.retries (counter) - Number of retry attempts
 *
...
public class TransferTelemetryMetrics {
    ...
    private static final String METER_NAME = "qraft-core";
    ...
    private TransferTelemetryMetrics() {
        ...
        // Initialize counters
        transfersTotal = meter.counterBuilder("qraft.transfer.total")
                .setDescription("Total number of transfers initiated")
                .setUnit("1")
                .build();

        transfersCompleted = meter.counterBuilder("qraft.transfer.completed")
                .setDescription("Number of successfully completed transfers")
                .setUnit("1")
                .build();

        transfersFailed = meter.counterBuilder("qraft.transfer.failed")
                .setDescription("Number of failed transfers")
                .setUnit("1")
                .build();

        transfersCancelled = meter.counterBuilder("qraft.transfer.cancelled")
                .setDescription("Number of cancelled transfers")
                .setUnit("1")
                .build();

        bytesTransferred = meter.counterBuilder("qraft.transfer.bytes.total")
                .setDescription("Total bytes transferred")
                .setUnit("By")
                .build();

        retryAttempts = meter.counterBuilder("qraft.transfer.retries")
                .setDescription("Number of transfer retry attempts")
                .setUnit("1")
                .build();

        // Initialize histograms
        transferDuration = meter.histogramBuilder("qraft.transfer.duration.seconds")
                .setDescription("Transfer duration in seconds")
                .setUnit("s")
                .build();

        transferThroughput = meter.histogramBuilder("qraft.transfer.throughput.bytes_per_second")
                .setDescription("Transfer throughput in bytes per second")
                .setUnit("By/s")
                .build();

        // Initialize gauges
        meter.gaugeBuilder("qraft.transfer.active")
