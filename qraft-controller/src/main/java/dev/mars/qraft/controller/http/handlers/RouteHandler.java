...
import dev.mars.qraft.controller.state.RouteCommand;
...
public class RouteHandler {
    ...
    public RouteHandler(RaftNode raftNode, QraftStateStore stateStore) {
    ...
    public Handler<RoutingContext> handleCreate() {
        return ctx -> {
            try {
                ...
                if (body == null) {
                    throw QraftApiException.badRequest(ErrorCode.BAD_REQUEST, "Request body is required");
                }
                ...
                QraftStateStore stateMachine = this.stateStore;
                if (stateMachine.hasRoute(routeToStore.getRouteId())) {
                    throw QraftApiException.conflict(ErrorCode.ROUTE_DUPLICATE, routeToStore.getRouteId());
                }
                ...
                raftNode.submitCommand(command)
                        .onSuccess(result -> {
                            if (result instanceof CommandResult.NotFound<?> nf) {
                                ...
                                ctx.fail(QraftApiException.notFound(ErrorCode.ROUTE_NOT_FOUND, nf.id()));
                            } else {
    ...
    public Handler<RoutingContext> handleGet() {
        return ctx -> {
            ...
            RouteConfiguration route = stateStore.findRoute(routeId)
                    .orElseThrow(() -> QraftApiException.notFound(ErrorCode.ROUTE_NOT_FOUND, routeId));
        ...
        return ctx -> {
            try {
                ...
                if (existing.getStatus() == RouteStatus.TRANSFERRING || existing.getStatus() == RouteStatus.DELETED) {
                    throw QraftApiException.conflict(ErrorCode.ROUTE_STATE_CONFLICT,
                ...
                raftNode.submitCommand(command)
                        .onSuccess(result -> {
                            if (result instanceof CommandResult.NotFound<?> nf) {
                                ...
                                ctx.fail(QraftApiException.conflict(ErrorCode.ROUTE_STATE_CONFLICT,
     ...
     *
     * @throws QraftApiException with ROUTE_STATE_CONFLICT if the transition is invalid
     */
    ...
    private void validateRouteConfiguration(RouteConfiguration route) {
        try {
            ...
            throw QraftApiException.badRequest(ErrorCode.MISSING_REQUIRED_FIELD, fieldName);
