...
public class HeartbeatHandler implements Handler<RoutingContext> {
    ...
    public HeartbeatHandler(RaftNode raftNode, QraftStateStore stateStore) {
    ...
    public void handle(RoutingContext ctx) {
        try {
            ...
            stateStore.findAgent(agentId)
                    .orElseThrow(() -> QraftApiException.notFound(ErrorCode.AGENT_NOT_FOUND, agentId));
            ...
            raftNode.submitCommand(command)
                    .onSuccess(result -> {
                        if (result instanceof CommandResult.NotFound<?> nf) {
                            ...
                            ctx.fail(QraftApiException.notFound(ErrorCode.AGENT_NOT_FOUND, nf.id()));
                            return;
                        }
