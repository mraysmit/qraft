...
import dev.mars.qraft.core.JobPriority;
import dev.mars.qraft.core.JobAssignment;
import dev.mars.qraft.core.QueuedJob;
import dev.mars.qraft.core.RouteConfiguration;
import dev.mars.qraft.core.RouteStatus;
...
/**
 * Description for QraftStateStore
 *
...
public class QraftStateStore implements RaftLogApplicator {

    private static final Logger logger = LoggerFactory.getLogger(QraftStateStore.class);
    ...
    public CommandResult<?> apply(RaftCommand command) {
        ...
        try {
            CommandResult<?> result = switch (command) {
                ...
                case DistributedStateRaftCommand cmd -> applyDistributedStateCommand(cmd.delegate());
                ...
                default -> throw new IllegalArgumentException(
                        "Unsupported RaftCommand type: " + command.getClass().getName());
            };
    ...
    private CommandResult<?> applyDistributedStateCommand(DistributedStateCommand command) {
        String key = command.key();
        logger.debug("Processing distributed state command: key={}, type={}", key, command.getClass().getSimpleName());

        return switch (command) {
            case DistributedStateCommand.Put cmd -> {
            ...
            case DistributedStateCommand.Delete ignored -> {
    ...
    public byte[] takeSnapshot() {
        logger.debug("Taking state machine snapshot: agents={}, metadata={}, assignments={}, queue={}",
            agents.size(), systemMetadata.size(), jobAssignments.size(), jobQueue.size());
        try {
            QraftSnapshot snapshot = new QraftSnapshot();
            ...
            logger.info("Created snapshot: size={}bytes, agents={}, assignments={}, queue={}, routes={}, lastAppliedIndex={}",
                data.length, agents.size(), jobAssignments.size(), jobQueue.size(), routes.size(), lastAppliedIndex.get());
            return data;
        } catch (IOException e) {
            logger.error("Failed to create snapshot: agents={}, message={}", agents.size(), e.getMessage());
    ...
    public void restoreSnapshot(byte[] snapshot) {
        ...
        try {
            QraftSnapshot restoredSnapshot = objectMapper.readValue(snapshot, QraftSnapshot.class);
            ...
            logger.info("Restored snapshot: agents={}, metadata={}, assignments={}, queue={}, routes={}, lastAppliedIndex={}",
                agents.size(), systemMetadata.size(),
    ...
    @Override
    public void reset() {
        logger.debug("Resetting state machine: agents={}, metadata={}",
            agents.size(), systemMetadata.size());
        agents.clear();
        systemMetadata.clear();
        jobAssignments.clear();
