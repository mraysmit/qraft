...
public class JobAssignmentHandler {
    ...
    public JobAssignmentHandler(RaftNode raftNode, QraftStateStore stateStore) {
                ...
                raftNode.submitCommand(command)
                        .onSuccess(result -> {
                            if (result instanceof CommandResult.NotFound<?> nf) {
                                ...
                                ctx.fail(QraftApiException.notFound(ErrorCode.ASSIGNMENT_NOT_FOUND, nf.id()));
                            } else {
    ...
    public Handler<RoutingContext> handleGet() {
        return ctx -> {
            ...
            JobAssignment assignment = stateStore.findJobAssignment(assignmentId)
                    .orElseThrow(() -> QraftApiException.notFound(ErrorCode.ASSIGNMENT_NOT_FOUND, assignmentId));
            ...
            raftNode.submitCommand(command)
                    .onSuccess(result -> {
                        if (result instanceof CommandResult.NotFound<?> nf) {
                            ...
                            ctx.fail(QraftApiException.conflict(ErrorCode.ASSIGNMENT_STATE_CONFLICT,
        ...
        return ctx -> {
            try {
                ...
                if (body == null || !body.containsKey("status")) {
                    throw QraftApiException.badRequest(ErrorCode.MISSING_REQUIRED_FIELD, "status");
                }
     ...
     *
     * @throws QraftApiException with ASSIGNMENT_NOT_FOUND if it doesn't exist
     */
     ...
     *
     * @throws QraftApiException with ASSIGNMENT_STATE_CONFLICT if the transition is invalid
     */
