...
class HttpApiServerHealthTest {
    ...
    static void setUp() throws Exception {
        ...
        raftNode = RaftNode.builder().vertx(vertx).nodeId("health-test-node").clusterNodes(clusterNodes).transport(transport).stateMachine(stateMachine).mode(RaftNodeMode.volatileMode()).commandCodec(new ProtobufRaftCommandCodec())
    ...
    class ErrorCorrelationTests {
        ...
        void shouldPropagateRequestIdInErrorResponse(VertxTestContext ctx) {
            ...
            webClient.get(HTTP_PORT, "localhost", "/api/v1/transfers/nonexistent-job-xyz")
                    ...
                    .send()
                    .onComplete(ctx.succeeding(response -> ctx.verify(() -> {
                        // Should get a 404 error from QraftApiException.notFound
