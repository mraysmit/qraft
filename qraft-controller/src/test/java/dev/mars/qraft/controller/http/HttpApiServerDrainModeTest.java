...
class HttpApiServerDrainModeTest {
    ...
    static void setUp() throws Exception {
        ...
        raftNode = RaftNode.builder().vertx(vertx).nodeId("drain-test-node").clusterNodes(clusterNodes).transport(transport).stateMachine(stateMachine).mode(RaftNodeMode.volatileMode()).commandCodec(new ProtobufRaftCommandCodec())
