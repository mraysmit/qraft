...
import dev.mars.qraft.agent.config.AgentConfig;
import dev.mars.qraft.agent.config.AgentConfiguration;
import dev.mars.qraft.agent.observability.AgentMetrics;
import dev.mars.qraft.agent.observability.AgentTelemetryConfig;
import dev.mars.qraft.agent.service.AgentRegistrationService;
import dev.mars.qraft.agent.service.HeartbeatService;
import dev.mars.qraft.agent.service.TransferExecutionService;
import dev.mars.qraft.agent.service.HealthService;
import dev.mars.qraft.agent.service.JobPollingService;
import dev.mars.qraft.agent.service.JobStatusReportingService;
...
/**
 * Main class for the Qraft Agent.
 * This agent registers with the Qraft controller cluster and executes file transfer tasks.
 *
...
public class QraftAgent {
    ...
    /**
     * Creates a QraftAgent with Vert.x dependency injection.
     *
    ...
    private QraftAgent(Vertx vertx, AgentConfiguration config, boolean closeVertxOnShutdown) {
        ...
        logger.info("Creating QraftAgent with Vert.x instance: {} (using Vert.x timers, no ScheduledExecutorService)",
    ...
    public static void main(String[] args) {
        ...
        logger.info("Starting Qraft Agent with OpenTelemetry...");
        ...
        try {
            ...
            logger.error("Failed to start Qraft Agent: {}", e.getMessage());
            logger.debug("Stack trace", e);
            System.exit(1);
        }

        logger.info("Qraft Agent stopped");
    }
    ...
    public void start() throws Exception {
        ...
        logger.info("Starting Qraft Agent services (Vert.x reactive mode)...");
    ...
    private void startBackgroundServices() {
        ...
        logger.info("Qraft Agent started successfully (reactive mode, 0 extra threads)");
    }
    ...
    public void shutdown() {
        ...
        logger.info("Shutting down Qraft Agent (reactive mode)...");
        running = false;
        ...
        try {
            ...
            healthService.shutdown()
                ...
                .onComplete(ar -> {
                    ...
                    logger.info("Qraft Agent shutdown complete (0 threads to cleanup)");
